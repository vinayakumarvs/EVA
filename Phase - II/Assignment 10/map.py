...
file_name = "%s_%s_%s" % ("TD3", "CarModel", str(seed))
print ("---------------------------------------")
...
# destinations
destinations = [[570,340],[1048,470],[1315,380]]
...
def init():
    ...
    goal_x, goal_y = destinations[np.random.randint(0, 3)]
    # goal_x = 1220
    # goal_y = 622
    first_update = False
    ...
    def step(self,action):
        ...
        if self.distance < 30:
            if self.topickup == 1:
                ...
                self.topickup = 1
                last_reward = 100
                self.pickup.size = 35, 35
                self.drop.size = 1, 1
                done = True

        # Collect item
        if self.topickup == 0:
            self.pickup.x = self.car.x - 10
            self.pickup.y = self.car.y - 10
